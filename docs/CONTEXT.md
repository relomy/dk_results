...
- **VipPresence** — the oracle answering whether *any* tracked VIP is entered in a contest, returning a presence verdict. Consults the standings poll `SportProcessor` already persists first (`present_vips`, per-VIP once a contest has been polled), and falls back to an entrant-page scan only when nothing's been polled yet. The entrant scan short-circuits at the first tracked VIP found and never enumerates the full entrant list — it proves presence/absence but never the complete roster of who's in, unlike the standings poll, which is per-VIP-accurate for the whole field once available. See ADR-0012. Module: `notifications/vip_presence.py`.
