...
- **Optimizer** — the module that solves for a single sport's optimal lineup via linear programming (PuLP/GLPK). Constructed with a `Sport` and a `dict[str, Player]`; its entire public interface is `get_optimal_lineup() -> list[Player] | None` (the LP-construction steps — decision variables, objective, budget/count/position constraints, solve, extract — are private). Module: `analytics/optimizer.py`.
