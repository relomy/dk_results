...
- **Optimizer eligibility** — whether a sport's lineups are optimized, carried by `Sport.allow_optimizer`. Defaults to `False` (opt-in): a sport is optimized only once its `positions` layout is confirmed against real DraftKings data, so an unverified sport never silently ships a wrong lineup. Enabled: NFL, NBA, MLB, SOC, CFB, GOLF. XFL/USFL and other unconfirmed sports are held off. See ADR-0005.
