...
- **Start-time bucket** — a group of lobby contests sharing a calendar date and the same displayed local start time at `HH:MM` precision. Used by the `dkcontests` report to distinguish same-day slates without changing contest selection or persistence behavior.
- **Featured draft group** — a raw DraftKings draft group whose `DraftGroupTag` is exactly `Featured`.
- **Lobby feed** — the anonymous DraftKings lobby payloads for a sport: `getcontests?sport=` (a dict of `Contests` + `DraftGroups`, upcoming) and `getlivecontests?sport=` (a list of in-progress `Contests`). Read without authentication; `get_lobby_response` (`lobby/fetch.py`) constructs no `AuthSession` for it. Parsed by `get_contests_from_response` / `filter_draft_groups`. Distinct from `ContestResultsPort` (authenticated, one contest's live readouts) and `Contest` (one parsed lobby row). See ADR-0009.
  _Avoid_: lobby row (that is a single `Contest`).
...
- **Cash line** — the last paid position and its score in a contest's standings: `(min_rank, min_cash_pts)`, computed by `_update_cash_stats` from `positions_paid` and each row's rank/points as standings are parsed. Owned by `ContestStandings` (`domain/contest_standings.py`); persisted onto `ContestDatabase.contests`, overwritten every `SportProcessor` poll. A positional/scoring cutoff, not a realized-payout fact — see ADR-0011.
  _Avoid_: conflating with **Cashing** — the cash line only tells you where the cutoff sits, not whether a given entry's payout is confirmed.
- **Projected cashing** — a tracked VIP's standings row sits at or above the current **cash line** (`rank <= cash line rank`), as of the last `SportProcessor` poll. A provisional, repeatedly-refreshed estimate surfaced as a bullet in the bot's `!live` command; the verdict itself is computed at read time, but each VIP's own rank/points is persisted (`VipCashStatus`, one row per tracked VIP per contest — see below), since `!live` runs in the bot process and has no other way to see per-VIP standings. Distinct from **Cashing**, which is the authoritative, realized-payout fact only available via `CompletionProcessor`'s soft-finish leaderboard check. See ADR-0011.
- **VipCashStatus** — a named frozen shape (`vip_name`, `rank`, `points`) for one tracked VIP's standings row in one contest, following the `ContestRow` precedent (ADR-0006) of a named shape over positional tuples. Written via `ContestDatabase.replace_vip_cash_status(dk_id, statuses)` — one call per `SportProcessor` poll that atomically replaces the full set for that contest, matching how the whole VIP list is re-parsed each cycle rather than updated incrementally. Read via `ContestDatabase.get_vip_cash_status(dk_id)`. See ADR-0011.
...
- **Optimizer eligibility** — whether a sport's lineups are optimized, carried by `Sport.allow_optimizer`. Defaults to `False` (opt-in): a sport is optimized only once its `positions` layout is confirmed against real DraftKings data, so an unverified sport never silently ships a wrong lineup. Enabled: NFL, NBA, MLB, SOC, CFB, CFBNight, GOLF, WeekendGolf. A narrow sport variant (see below) inherits its parent sport's confirmed `positions` layout rather than needing independent confirmation — CFBNight reuses CFBSport's. Showdown variants stay off pending a captain-multiplier model, and XFL/USFL and other unconfirmed sports are held off. See ADR-0005.
- **Narrow sport variant** — a `Sport` subclass that shares a parent sport's DraftKings lobby feed (`draftkings_sport` set to the parent's `name`) but scopes to one slate within it via `suffixes` matching against the lobby contest-name suffix, with `allow_suffixless_draft_groups = False` so it never absorbs the parent's unsuffixed draft groups. Established by `NFLAfternoonSport` (parent: NFL) and `CFBNightSport` (parent: CFB). Deliberately excluded from `DISCORD_ROLE_MAP` (no double-up-found pings) and `bonus_rules.py`/the non-cashing list (no bonus tracking) unless a variant specifically needs them — narrower slates piggyback on the parent sport's wiring there rather than duplicating it by default.
  _Avoid_: Showdown variants (`NFLShowdownSport`, `SOCShowdownSport`, `PGAShowdownSport`) — those have their own distinct roster shape (a CPT slot), not just a narrower slice of the same roster, so they aren't "narrow sport variant" in this sense even though they also key off `suffixes`.

- **SportProcessor** — the module that owns the full "process one sport, write to sheet" workflow. Public interface: `SportProcessor.run(sport_name, sport_cls) -> int`. Three injected ports: `DkPort` (DraftKings HTTP), `SheetPort` (Google Sheets, via `sheet_factory: Callable[[str], SheetPort]`), `BonusSenderPort` (Discord). `ContestDatabase` is injected directly (local-substitutable): read to find the live contest (`get_live_contest`), and written each run with the parsed standings' **cash line** (see ADR-0011) — its only write path into that table. Raises `NoLiveContestError`, `StandingsUnavailableError`, or `StandsParseError` when a sport must be skipped. Module: `sport_processor.py`.
...
- **Runtime bootstrap** — the one startup operation each executable performs: load the repository `.env` through `python-dotenv` without overriding a non-empty process value, resolve repository configuration, and apply values only where the process environment is unset or empty. Its public interface is `config.load_and_apply_settings()`; library imports do not perform it.
  _Avoid_: the redundant `dotenv` package; per-entry-point dotenv setup; import-time configuration.
...
- **ContestResultsPort** — the seam through which the completion workflow reads one contest's live DraftKings readouts — state, entrants, leaderboard — by `dk_id`.
  _Avoid_: Lobby feed (the anonymous per-sport listing of new contests); stored contest state (`ContestDatabase.get_contest_state`).
- **Announcement** — the shared visual chrome (sport emoji, header line, bulleted detail lines, inline sheet link) assembled once and reused by all 6 push-notification kinds: contest-warning, live, completed, soft-finish, bonus-achievement, double-up-found — plus the `!contests`/`!live` bot commands, which render through the same `build_milestone_announcement` under a `"Live"` prefix. Each `build_*` function in `discord_announcements.py` supplies only the fields relevant to its kind — no forced empty/placeholder fields. `build_milestone_announcement` also takes an optional `vip_presence` ("present"/"absent"/`None`) that appends a trailing `⭐ VIP: ...` bullet when set; only `!contests`/`!live` populate it, reading the cached verdict via `NotificationStore.get_presence` — the four automated milestone announcements leave it unset. `DISCORD_ROLE_MAP` (sport → emoji, role mention) lives alongside the builders; it also gates the double-up-found kind, which sends nothing at all for a sport absent from the map (intentional, existing scope — not a bug). Distinct from **Notification event**/`NotificationStore` above, which is dedup identity, not message content. Module: `discord_announcements.py`. See ADR-0010.
  _Avoid_: Notification event (the dedup identity, not the message text).
