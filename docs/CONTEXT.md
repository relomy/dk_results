...
- **Draft Group Filter** — the module (`lobby/draft_group_filter.py`) that owns all sport-specific draft-group qualification logic: tag filtering, game-type constraint, suffix matching, time constraint, and NFLShowdown deduplication. Public interface: `filter_draft_groups(groups, sport) -> list[int]`. Suffix matching reads a single `Sport.suffixes: tuple[str | None, ...] | None` field: `None` is unconstrained, `()` requires suffixless, a tuple of patterns requires a match and rejects suffixless, and a tuple including `None` accepts either. `NFLSport` is suffixless-only (`suffixes = ()`); `NFLAfternoonSport` matches `(Afternoon Only)` and is scoped to Classic (`contest_restraint_game_type_id = 1`) so Tiers/Snake draft groups sharing that suffix text aren't misrouted into it. See ADR-0012. `NFLShowdownSport` additionally sets `contest_restraint_time = time(18, 0)`: DraftKings tags one `Featured` Showdown per game window (not just primetime), so a 6:00 PM ET floor keeps only the single-game primetime/marquee slate (Thu/Sun/Mon night, Super Bowl) and excludes the day-window Featured picks that share identical suffix/game-type shape. See ADR-0013.
