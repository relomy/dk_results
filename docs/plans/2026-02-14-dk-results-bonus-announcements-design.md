...
known tokens to avoid false positives.
...
Normalize player names using existing `normalize_name` behavior.
...
Constraints:
- `UNIQUE (contest_id, sport, player_name, bonus_code)`

## Concurrency and Failure Handling
Avoid holding transactions open while sending webhooks. Use a compare-and-set
update after successful sends:

1. Read `old_count`.
2. Send webhook messages.
3. `UPDATE ... SET last_announced_count = ?, updated_at = ... WHERE
...
## Message Formatting
Include VIP usernames (cap to 5, then `+X more`).
