...
class SheetPort(Protocol):
    ...
    def write_players(self, values: Any, format_plan: Any = ()) -> None: ...
...
class SportProcessor:
    ...
    def run(self, sport_name: str, sport_cls: type[Sport]) -> int:
        ...
        self._persist_cash_status(int(dk_id), results)
    ...
    def _persist_cash_status(self, dk_id: int, results: ContestStandings) -> None:
        """
    ...
    def _write_standings(
        ...
        values, format_plan = players_to_values(results.players, sport_name)
        sheet.write_players(values, format_plan)
